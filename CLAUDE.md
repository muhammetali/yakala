...
- **Disposable lifecycle ownership:** anything with a `dispose()` method (`TextEditingController`, `FocusNode`, `AnimationController`, `ChangeNotifier` listeners) MUST be owned by a `StatefulWidget`'s `State` or service that disposes it in its own `dispose()`. **Never** create one as a local in an async function and dispose after `await showDialog/Navigator.push` — the route's exit transition still rebuilds child widgets that touch the controller, leading to "used after dispose" assertions. See `lib/widgets/text_input_dialog.dart` for the canonical pattern (controller in `initState`, disposed in `State.dispose` — framework guarantees disposal happens AFTER the widget fully leaves the tree, including animations). When you need a quick text prompt, reuse `TextInputDialog.show()`.
- **Concurrency invariants for orchestrators:** services that drive shared OS state (window mode, OverlayController completers, tray icon state) must enforce single-flight. Rule: if `serviceX.startSomething()` mutates global state and awaits user interaction, a second concurrent call must be either rejected upfront or fully serialized — never run both halves and rely on cleanup ordering. `CaptureService.capture()` uses an `_inFlight` mutex (returns `CaptureResult.cancelled()` synchronously when busy) because the alternative — letting the second call's `enterOverlay()` run before the first's `finally`-block `exitOverlay()` — produced the "editor opens but window is invisible" bug. Apply the same pattern to any future capture-like flow.
